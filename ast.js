...
function convertToTerm(ast) {
	...
	for(var prop in ast) {
		...
		if(ast.hasOwnProperty(prop)){
			...
			elems.push(AST.mk(prop, arr));
		    } else {
			elems.push(AST.mk(prop, [convertToTerm(value)]));
		    }

		}
	    }
	}
	return AST.mk(ast.type, elems);
    }
    return ast;
}

var f1_old = parse("g(f(42,43))");
var f1_new = parse("g(f(43))");

var f2_old = parse("f(44,117)");
var f2_new = parse("f(117)");
...
var conv_rewrites = AST.getMergeDiffs(conv_changeset);
console.log("rewrites 2:");
if(conv_rewrites) {
